...
test( 'main export is a function', function test( t ) {
	t.ok( typeof revive === 'function', 'main export is a function' );
	t.end();
});

test( 'values which are not recognized as serialized error objects are unaffected', function test( t ) {
	var expected;
	var actual;
...
test( 'an object must have a recognized "type" field in order to be revived', function test( t ) {
	var expected;
	var actual;
	var json;
...
test( 'an object must have a "message" field in order to be revived', function test( t ) {
	var expected;
	var actual;
	var json;
...
test( 'the function will revive a JSON-serialized error object', function test( t ) {
	var expected;
	var actual;
	var types;
	var ctors;
	...
	for ( i = 0; i < types.length; i++ ) {
		...
		t.equal( actual.stack, expected.stack );
	}
	t.end();
});

test( 'non-standard error properties are bound to the revived error instance', function test( t ) {
	var json;
	var err;
	...
	err = JSON.parse( JSON.stringify( json ), revive );

	t.equal( err.beep, json.beep, 'shallow properties' );

	t.notEqual( err.arr, json.arr, 'separate instances' );
	assert.deepEqual( err.arr, json.arr );
	t.ok( true, 'deep equal' );

	t.end();
});

test( 'if a serialized error object lacks a "stack" field or has a "stack" field and lacks a stack trace, the returned error will not have an associated stack trace', function test( t ) {
	var json;
	var err;
...
test( 'the function will revive deeply nested serialized error objects', function test( t ) {
	var expected;
	var actual;
	var ctors;
	var json;
